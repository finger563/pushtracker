...
static void (*pTimer0Callback)(void);
static void (*pTimer1Callback)(void);
...
void LogData(void)
{
	...
	log_string("%Time(ms),%Filt_Y,%PushTimes,%RawY,%RawX\n");
	rprintf("Logging...\n");
	...
	while (_read_state != 1)
	{
		if(LogFlag==TRUE)
		{
			if (initialize<RAW_BUFFER_SIZE)
			{
				...
				PT_struct.time_buffer[initialize] = getSystemTime();
				...
				IO_PORT_PIN = IO_PORT_PIN ^ (1<<BLUE);
				sprintf(str_data, "%05d,%05f,0,%05d,%05d,0,0\n",systemTimeMs,PT_struct.filtered_buffer[initialize],accelY(),accelX());
				log_string(str_data);
				
				_read_state = (~(IO_PORT_PIN>>SWITCH_ENTER))&0x01;
			...
			else
			{
				...
				if ((PT_struct.max_mask&0x01) == 0)		// to see if there is a new max
				{
					...
					{
						if (PT_struct.max_times[VAL_BUFFER_SIZE-1]>PT_struct.min_times[VAL_BUFFER_SIZE-1]&& (PT_struct.max_times[VAL_BUFFER_SIZE-1]-PT_struct.min_times[VAL_BUFFER_SIZE-1])<3000)		// && (PT_struct.max_times[VAL_BUFFER_SIZE-1]-PT_struct.min_times[VAL_BUFFER_SIZE-1])<1500
						{
				...
				PT_struct.time_buffer[RAW_BUFFER_SIZE-1] = getSystemTime();
				...
				if (PushFlag==TRUE)
				{ 
					sprintf(str_data, "%05d,%05f,%05d,%05d,%05d,%05d,%05d\n",systemTimeMs,PT_struct.filtered_buffer[RAW_BUFFER_SIZE-1],PT_struct.push_times[RAF_BUFFER_SIZE-1],accelY(),accelX(),PT_struct.min_times[VAL_BUFFER_SIZE-1],PT_struct.max_times[VAL_BUFFER_SIZE-1]); 
					PushFlag=FALSE;
				}
				else sprintf(str_data, "%05d,%05f,0,%05d,%05d,0,0\n",systemTimeMs,PT_struct.filtered_buffer[RAW_BUFFER_SIZE-1],accelY(),accelX());
				log_string(str_data);
...
{	
	...
	//initTimer1(timer1ISR);		// initialize timer 1
	initTimer0(timer0ISR);
	
	IO_PORT_DIR |= (1<<BLUE)|(1<<USB_LED);							// set BLUE and USB/GREEN leds as output
	IO_PORT_CLR = (1<<BLUE)|(1<<USB_LED); 									//Turn on USB LED
	//FIO0DIR |= 1<<BLUE;
	//FIO0CLR = 1<<BLUE;
	...
	IO_PORT_SET = (1<<USB_LED)|(1<<BLUE); 		//Turn off USB LED
	//FIO0SET = 1<<BLUE;
}
...
void card_init(void)
{
	if(!sd_raw_init()) error_mode(2); 		//Attempt to init SD raw mode

	if(openroot()) error_mode(5);		//Attempt to open root directory
}
